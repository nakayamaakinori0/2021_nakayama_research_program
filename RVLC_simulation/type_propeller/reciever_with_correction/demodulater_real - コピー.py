...
for d in range(10, 55, 5):
    ...
    for shifted in range(0, 360, 1):
        ...
        # 中心座標取得
        detected_center_x, detected_center_y = tpp.getCtr(im_all_light_gray)
        #detected_center_x = 1002
        #detected_center_y = 1000
        # 中心座標補正
        corrcted_ctr_x, corrected_ctr_y = tpp.correctCtr(detected_center_x, detected_center_y, im_all_light_gray, codn_threshold)
         # 座標取得
        codn_x_array, codn_y_array = tpp.getCodn(im_all_light_gray, initial_phase, codn_threshold, corrcted_ctr_x, corrected_ctr_y)
    ...
    center_x_arr = np.append(center_x_arr, corrcted_ctr_x)
    center_y_arr = np.append(center_y_arr, corrected_ctr_y)
