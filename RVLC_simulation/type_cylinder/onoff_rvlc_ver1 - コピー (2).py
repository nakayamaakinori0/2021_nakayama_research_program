...
if(l_hight==0):#四捨五入したら０になってもて信号縦にくっついてまうからその予防
    l_hight=1
...
for degree in range(0,181,step):
    ...
    dif_ls = (int) (dif_ls)
    dif_le = (int) (dif_le)
    list_dif_ls.append(dif_ls)
    list_dif_le.append(dif_le)
    list_n_p.append(n_p)
    list_n_p_l.append(n_p_l)
    #print(dif_ls)
    #print(dif_le)
        ...
        for l in range(10):
            ...
            if(s==1):
                for y_l in range(l_hight):
                    for x_l in range(0-dif_ls, n_p_l, 1):
                        #print(x_l)
...
filename = f'./tonoff_rvlc_{d}m.png'
