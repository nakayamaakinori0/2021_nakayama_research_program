...
header4_dir = "header4"
footer_dir = "footer"
protocol_frame = [2,1,60,10,20,2]
trimming_heada2_dir = "trimming_header2"
data_dir = "data"

list_input_img = mm.img_to_list(input_dir, 0, 98, 1)
...
for i in range(0, len(list_input_img)):
    ...
    if i < sum(protocol_frame[:1]):
        ...
        cv2.imwrite(header4_dir+'/'+str(i)+ '.png', list_input_img[i])
    elif i < sum(protocol_frame[:5]):
        cv2.imwrite(data_dir+'/'+str(i)+ '.png', list_input_img[i])
    elif i < sum(protocol_frame[:6]):
