...
recieved_signal_by_header4 = np.loadtxt(
        fname = "recieved_signal_by_header4.csv",
        dtype = "int",
        delimiter = ","
        )
...
for i in range(0, len(transmitted_signal)):
    if transmitted_signal[i] != recieved_signal_by_header3[i]:
        error_by_header3 += 1
    if transmitted_signal[i] != recieved_signal_by_header4[i]:
        error_by_header4 += 1

BER_by_header3 = mm.culcBER(error_by_header3, num_data_part, num_LED, data_range)
BER_by_header4 = mm.culcBER(error_by_header4, num_data_part, num_LED, data_range)
