...
for input_file_name in data_path:
    ...
    for i in range(0, len(coordinate_by_header3_array)):
        ...
        picked_pixel_value_by_header4 = np.append(picked_pixel_value_by_header4,img_data[ycoordinate_by_header4,xcoordinate_by_header4])

picked_pixel_value_by_header3 = np.delete(picked_pixel_value_by_header3,0,0)
picked_pixel_value_by_header4 = np.delete(picked_pixel_value_by_header4,0,0)
np.savetxt("picked_pixel_value_by_header3.csv", picked_pixel_value_by_header3 ,fmt="%d", delimiter=",")
np.savetxt("picked_pixel_value_by_header4.csv", picked_pixel_value_by_header4 ,fmt="%d", delimiter=",")
